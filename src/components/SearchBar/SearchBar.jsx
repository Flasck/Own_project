...
{
	const words = normalizeString(text).split(" ");
	return projects.filter(v =>
	{
		if (tech !== null && v.technologies.indexOf(tech) < 0)
			return false;

		for (const word of words)
		{
			if (v.description.toLowerCase().indexOf(word) < 0
				&& v.title.toLowerCase().indexOf(word) < 0)
				return false;
		}
		return true;
	});
